...
export default function SSEComponent() {
    ...
    useEffect(() => {
        ...
        // Send the SSE URL to the worker to start listening
        sseWorkerRef.current.postMessage({url: `${import.meta.env.VITE_API_URL}/evaluations/stream`});
        ...
        sseWorkerRef.current.onmessage = (event) => {
            switch (event.data.type) {
                case "sse_connected":
                    ...
                    break;
                case "sse_message":
                    console.log("SSE message received:", event.data.message);
